...
| `FileUpload`  | **The same shape as `ComboBox`'s toggle, a second time.** The base puts a drop zone's labelling on a visually hidden button inside it — `aria-label="DropZone"` plus an `aria-labelledby` that references itself and then whatever we passed — so the name comes out as "DropZone Attachments". Measured, and left alone for the reason that one was: the base is gluing its own word to ours through element references, and it does say what the thing is. Whether a person hears one useful name or two things stuck together is what a person hears                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
